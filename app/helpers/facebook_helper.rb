...
FACEBOOK_URL = "http://facebook.com"
  ...
  def post_vtag(new_video, friends_ids_arr, video_title)
    if friends_ids_arr.any?
      users_message_state = new_video ? "has Vtagged a new video" : "has updated a VtagO"
      post_on_users(users_message_state, video_title)
      post_on_friends(friends_ids_arr, video_title)
    end
  end

  def post_on_users(message_part, video_title)
  ...
  def post_on_friends(friends_ids_arr, video_title)
    fb_graph.put_wall_post("",
                            {
	                            "name" => "VtagO - #{video_title}",
	                            "link" => "http://www.ynet.co.il",
